...
namespace CTS
{
    ...
    public class CTSDemoController : MonoBehaviour
    {
        ...
        private Quaternion m_walkRotation;

        [Header("Fly Controller")]
        public CTSFly m_flyController;
        private Vector3 m_flyPos;
        private Quaternion m_flyRotation;
        ...
        void Awake()
        {
            ...
            //Get initial location and rotation
            m_startPos = m_target.transform.position;
            m_startRotation = m_target.transform.rotation;
            m_flyPos = m_target.transform.position;
            m_flyRotation = m_target.transform.rotation;
            m_walkPos = m_target.transform.position;
            m_walkRotation = m_target.transform.rotation;
            ...
            if (m_instructions != null)
            {
                ...
                if (m_tesselatedProfile != null)
                {
                    if (commands.Length > 0)
                    {
                        commands += ", 4. Tesselated";
                    }
                    else
                    {
                        commands = "Controls: 4. Tesselated";
                    }
                }
                if (m_flyController != null)
                {
        ...
        public void Home()
        {
            ...
            m_target.transform.position = m_startPos;
            m_target.transform.rotation = m_startRotation;
        }
        ...
        public void Fly()
        {
            if (m_flyController != null)
            {
                if (!m_flyController.isActiveAndEnabled)
                {
                    ...
                    m_target.transform.position = m_flyPos;
                    m_target.transform.rotation = m_flyRotation;
                }
                else
                {
                    m_flyPos = m_target.transform.position;
        ...
        public void Walk()
        {
            if (m_walkController != null)
            {
                if (!m_walkController.isActiveAndEnabled)
                {
                    ...
                    m_target.transform.position = m_walkPos;
                    m_target.transform.rotation = m_walkRotation;
